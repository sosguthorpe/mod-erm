...
public class StringTemplatingService {
  ...
  private Set<TemplatedUrl> getTeplatedUrlsForRootBinding( final Map<String, List<StringTemplate>> templates, final StringTemplateBindings rootBindings ) {
    ...
    final Set<TemplatedUrl> generatedUrls = Stream.of( noneProxiedUrls )
      .flatMap({ final TemplatedUrl turl ->
        ...
        // Replace with a stream containing the original plus all the proxied variants
        Stream.concat(Stream.of(turl), addTemplateUrlMappings( templates.get(CONTEXT_PROXY).stream(), bindings ))
      })
  ...
  @CurrentTenant
  @Transactional(propagation = MANDATORY)
  protected List<String> executeTemplatesForSinglePTI (final PlatformTitleInstance pti, final Map<String, List<StringTemplate>> stringTemplates = null) {
    ...
    // Treat URLs as immutable so we can use timestamps to decide whether we need to recalculate
    ...
    // Return a copy so as not to pass the Persisted set
    pti.templatedUrls.collect()
  }
  ...
  private Stream<TemplatedUrl> addTemplateUrlMappings( final Stream<StringTemplate> templates, final StringTemplateBindings bindings ) {
    return templates
      .map({ final StringTemplate template ->  new TemplatedUrl(
  ...
  @CurrentTenant
  @Transactional
  public Map<String, List<StringTemplate>> findStringTemplatesForId(final String theId) {
    
    final List<StringTemplate> stringTemplates = GormUtils.gormStaticApi(StringTemplate).createCriteria().listDistinct {
      cache true
      readOnly true
      or {
        and {
          eq 'context.value', CONTEXT_CUSTOMIZER
          eq 'idScopes', theId
        }
        and {
          eq 'context.value', CONTEXT_PROXY
          notExists(new DetachedCriteria(StringTemplate, 'proxyExcludes').build {
            createAlias('proxyExcludes.idScopes', 'exclude')
            idEq 'this.id'
            eq 'exclude', theId
          })
        }
    ...
    templates.urlCustomisers = Stream.of(stringTemplates)
      .filter({ StringTemplate tmp ->
        switch (tmp.context?.value) {
          case CONTEXT_CUSTOMIZER:
            return true
            break
          case CONTEXT_PROXY:
            templates.urlProxiers.add(tmp)
            return false
            break
          default:
  ...
  public void generateUrlsForPtiWithId ( final String ptiId ) {
    ...
    final Promise<List<String>> deferred = WithPromises.task { 
      executor.execute({ final String tenantId, final String pti, final String lockId ->
